...
exports.getPurchaseHistory = async (req, res) => {
    ...
    try {
        if (!isNaN(affiliateId) && parseInt(userId) !== user) {
...
// Add affiliate name to each plan
            const plansWithAffiliate = await Promise.all(plans.map(async (plan) => {
